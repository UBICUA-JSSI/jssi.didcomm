...
class IssuerTest {
    ...
    void createCredentialKeyCorrectnessProof() {
        ...
        byte[] hash = CredentialHelper.hash(Bytes.concat(asUnsignedByteArray(z), asUnsignedByteArray(z_tilde)));
        BigInteger c = new BigInteger(1, hash);
        BigInteger x_cap = x_tilde.add(x.multiply(c));

        BigInteger z_inverse = z.modInverse(n);
        BigInteger z_cap_result = CredentialHelper.getPedersenCommitment(
                z_inverse,
                c,
                s,
                x_cap,
        ...
        byte[] hash_result = CredentialHelper.hash(Bytes.concat(asUnsignedByteArray(z), asUnsignedByteArray(z_cap_result)));
